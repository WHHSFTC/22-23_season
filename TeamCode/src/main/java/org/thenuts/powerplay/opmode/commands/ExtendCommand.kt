...
class ExtendCommand(val manip: Output, val target: VerticalSlides.Height, val outputSide: Output.OutputSide) : Command by mkSequential(strict = false, {
    val clearStop = target.pos < VerticalSlides.Height.MIN_CLEAR.pos && outputSide == Output.OutputSide.PASSTHRU
    if (clearStop) {
        add(LiftCommand(manip.lift, VerticalSlides.State.RunTo(VerticalSlides.Height.MIN_CLEAR.pos)))
    } else {
        add(LiftCommand(manip.lift, VerticalSlides.State.RunTo(target.pos + CONE_STEP * 2)))
    }
