...
abstract class AllNearHighAuto(right: Boolean): CyclingAuto(right) {
    override fun generateCommand(): Command {
        ...
        return mkSequential {
            add(samesideOutput(VerticalSlides.Height.HIGH.pos, startPose, samesidePose, aroundSignal = true))
...
class LeftAllNearHigh: NearHighAuto(false)

@Autonomous(preselectTeleOp = "OctoberTele", group = "_official")
class RightAllNearHigh: NearHighAuto(true)
